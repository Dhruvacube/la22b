...
def title_vote_limit(): return Settings.objects.values(
    'title_limit').get_or_create()[0]['title_limit']

# Create your views here.


def title_entry(request):
    title_model = Titles.objects.values(
        'title_name', 'slug', 'total_vote', 'desc', 'colour').all()
...
def vote(request, slug):
    ...
    if title_model.title_stu == 'ALL' and title_model.gender == 'ALL':
        student_model = Student.objects.filter(
            hidden=False).values('name', 'slug', 'class_stu').all()
        class_model = Student.objects.filter(hidden=False).values(
            'class_stu',).distinct('class_stu')

    elif title_model.title_stu == 'ALL':
        student_model = Student.objects.filter(
            hidden=False, gender=title_model.gender,).values('name', 'slug', 'class_stu').all()
        class_model = Student.objects.filter(
            hidden=False, gender=title_model.gender,).values('class_stu',).distinct('class_stu')

    elif title_model.gender == 'ALL':
        student_model = Student.objects.filter(
            hidden=False, class_stu=title_model.title_stu).values('name', 'slug', 'class_stu').all()
        class_model = Student.objects.filter(
            hidden=False, class_stu=title_model.title_stu).values('class_stu',).distinct('class_stu')

    else:
        student_model = Student.objects.filter(
            hidden=False, gender=title_model.gender, class_stu=title_model.title_stu).values('name', 'slug', 'class_stu')
        class_model = Student.objects.filter(hidden=False, gender=title_model.gender,
                                             class_stu=title_model.title_stu).values('class_stu',).distinct('class_stu')
    return render(
        request,
        'vote.html',
        {
...
def register_vote(request):
    if request.method == "POST":
        ...
        if request.session.get(student_dict, 0) > title_vote_limit():
            ...
            try:
                request.session[student_dict] = request.session[student_dict] + 1
            except:
                request.session[student_dict] = 1
            ...
            if Participants.objects.filter(student=student_model, title_part=title_model).exists():
                ...
                p.save()
            Titles.objects.filter(slug=title_slug).update(
                total_vote=F('total_vote') + 1)
