...
class SettingsAdmin(admin.ModelAdmin):
    search_fields = list_display = ('about_entry','title_limit','nickname_limit','confession_limit','vote_nicknameassigntime_start','vote_nicknameassigntime',)
...
class ContactAdmin(admin.ModelAdmin):
    search_fields = list_display = ('name', 'resolved', 'date_time','instagram_id', 'email',)
    list_filter = ('date_time','resolved')
    list_per_page = 20
    readonly_fields = ('date_time','instagram_id_url')
    fieldsets = (
        (_('Submitted by User'),{'fields':('name','instagram_id', 'email', 'subject', 'message')}),
